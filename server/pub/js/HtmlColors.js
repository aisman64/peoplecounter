...
(function(global) {
	...
	var lightValue = function(val, minVal) {
		// thresholding
		// return Math.min(val, maxVal);

		// scaling
		var actualMax = 255;
		var factor = minVal / actualMax;
		return Math.min(actualMax, minVal + Math.floor(val * factor));
	};
	...
	var HtmlColors = {
		 ...
		 * Make sure, we get something that is not too bright, so it will be visible when on white background
		 */
		getLightHex: function(idx, minVal) {
			minVal = minVal || 0;
			var color = this.array[idx];
			var r = lightValue(color.rgb.r, minVal);
			var g = lightValue(color.rgb.g, minVal);
			var b = lightValue(color.rgb.b, minVal);
			var hex = rgbToHex(r, g, b);
			return hex;
		},
